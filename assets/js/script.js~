...
// gototop & search & define
$(function() {

    console.log("学习就像谈恋爱一样，兴趣就是荷尔蒙，会让你对ta一见钟情。");

    $("#post").scroll(function() {
        $("#post").scrollTop() > 500 ? $("#rocket").addClass("show") : $("#rocket").removeClass("show");
    });
    ...
    old_content = $("#pl__container").html();
    $.ajax({
        url: "/search.xml",
        dataType: "xml",
		...
		keyword = $.trim($("#search-input").val());
 		$("#pl__container").html("");		
		if(keyword.length==0){
		    $("#pl__container").html(old_content);	
		}else{
                    result_count = 0;
                    $.each(data, function(){ 
			if(this.title.toLowerCase().indexOf(keyword.toLowerCase()) > -1){
				$("#pl__container").append('<a class="pl__all" href="'+this.url+'"><span class="pl__circle"></span><span class="pl__title">'+this.title+'</span><span class="pl__date">'+this.date+'</span></a>');
                                result_count++;
			}
		    });
                    if(result_count == 0){
				$("#pl__container").html("<div style='padding: 10px;'>未查询到数据</div>");
		    }
		}	
	    });
        }
